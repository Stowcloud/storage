...
// key identifies a watched directory relative to an owner. Host paths never
...
// addSticky pins k, lifting it out of the recent half. The first result says
// the caller must register a kernel watch; the second says the pin was accepted.
func (h *hotSet) addSticky(k key) (needsRegister, accepted bool) {
	if _, ok := h.sticky[k]; !ok && len(h.sticky) >= h.cap {
		return false, false
	}
	h.dropRecent(k)
	h.sticky[k]++
	return !h.isRegistered(k), true
}
